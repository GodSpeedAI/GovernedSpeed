...
Fine-grained Role-Based Access Control (RBAC) scopes access to platform resources:

| Role         | Permissions                                                       |
| ------------ | ----------------------------------------------------------------- |
| `developer`  | Read own build evidence, trigger CI checks.                       |
| `pmo`        | Read all dashboards, view risk snapshots, approve gate overrides. |
| `compliance` | Read/Export all evidence, manage policies (read-only by default). |
| `admin`      | Full access, manage users/roles, approve policy changes.          |
| `auditor`    | Read-only access to all evidence, policies, and audit exports.    |
