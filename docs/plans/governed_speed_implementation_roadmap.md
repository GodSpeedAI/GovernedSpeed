...
- **[Doc] Author Core ADRs** (Architecture Decision Records):
  - `ADR-001`: Governance as Code Pattern (Define SEA-DSL schema)
  ...
  - `ADR-008`: Security Model & Zero-Trust Access (SEA-DSL Roles)
  - `ADR-011`: **Prospection & Evolutionary Engine** (Defining the H1/H2/H3 horizons & Fibonacci scaling logic).
  - `ADR-012`: **Playbook & CMMN Architecture** (SEA-DSL Playbooks).
  - `ADR-013`: **Handbook as Code** (Generated from SEA-DSL).
- **[Doc] Draft Primary PRDs** (Product Requirements):
  ...
  - `PRD-009`: **Playbook & CMMN Execution Engine** (SEA-DSL Runtime).
  - `PRD-010`: **Governance Cockpit** (System Graph Explorer).
- **[Doc] Define System Specs (SDSs)**:
  ...
  - `SDS-012`: **Playbook Schema & CMMN Mapping** (SEA-DSL primitives).
...
1.  [x] **Create Directory**: `mkdir -p docs/architecture/{decisions,requirements,specs}`
2.  [x] **Draft ADR-001**: Define the SEA-DSL Policy-as-Code structure officially.
3.  [x] **Draft ADR-011**: Define the Prospection & Evolutionary Engine.
4.  [x] **Draft ADR-012**: Define the Playbook & CMMN Architecture.
5.  [x] **Draft PRD-008**: Define the Sensing Agents & Dashboard widgets.
6.  [x] **Draft PRD-009**: Define Playbook Engine & SEA-DSL Runtime.
7.  [x] **Draft SDS-012**: Playbook Schema & CMMN Mapping.
8.  [ ] **Repo Cleanup**: Organize existing `docs/Reference` content.
